...
public class Snippet {
				...
				public void paintControl(PaintEvent e) {
					...
					for (int i = 0; i < COLUMNS; i++) {
						for (int j = 0; j < ROWS; j++) {
							...
							gc.fillRectangle(i * (DIM + 1), j * (DIM + 1), DIM, DIM);
						}
					}
				}
			});
	...
	public static class SnakePanelRectangle {
		...
		public SnakePanelRectangle(SWTRepaintTimeline repaintTimeline) {
			this.backgroundColor = Display.getDefault().getSystemColor(SWT.COLOR_BLACK);
			this.isRollover = false;

			this.rolloverTimeline = new Timeline(this);
			this.rolloverTimeline.addPropertyToInterpolate("backgroundColor", Display.getDefault().getSystemColor(SWT.COLOR_YELLOW), Display
